...
Yv(1:(Nx-1)/4*3,1) = 10^(-6);
Yv((Nx-1)/4*3:(Nx-1),1) = 1-10^(-6);
...
while(time<final_time)
    ...
    % Update primitive flow variables after transport substep
    [rho,Yv,~,~,~,e,~,~,~] = updateFlowVariables(W,Nx);
    
    %% MASS TRANSFER SUBSTEP AFTER HYPERBOLIC STEP (PROBLEM SESSION 3) %%
    % MASS TRANSFER BY MEANS OF EQUILIBRIUM-BASED GIBBS FREE ENERGY RELAXATION 
    
    for j=1:Nx-1
        % Update mass fraction and state vector
        % Your Code Here
        
        Yv(j,1) = gibbsRelaxation(rho(j,1),e(j,1),Yv(j,1));
        W(2,j) = rho(j,1)*Yv(j,1);
        
    end
    
    %% %%
    
    % Update primitive flow variables after mass transfer substep
