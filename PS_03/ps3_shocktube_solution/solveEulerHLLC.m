...
%% Definition of states, fluxes, and natural variables on both sides of cell boundaries
    if (sign>0) % F_R from solveEulerHLLC
        ...
        rho_R = W(1,Row+sign);
        Yv_R = W(2,Row+sign)/rho_R;
		u_R = W(3,Row+sign)/rho_R;
		p_R = p(Row+sign,1);
        c_R = c(Row+sign,1);
        ...
        rho_L = W(1,Row+sign);
        Yv_L = W(2,Row+sign)/rho_L;
		u_L = W(3,Row+sign)/rho_L;
		p_L = p(Row+sign,1);
        c_L = c(Row+sign,1);
...
%% Calculation of states W_star_L and W_star_R at the cell boundaries (Toro (10.33))
    ...
    elseif(ind==2) % Vapor mass fraction
        W_star_L = rho_L*Yv_L*(S_L-u_L)/(S_L-S_star);
		W_star_R = rho_R*Yv_R*(S_R-u_R)/(S_R-S_star);
    elseif(ind==3) % Momentum
