%========================================================================
% code1.m
% Navier-Stokes solver for a drop falling in a rectangular
...
m0=0.01; % Viscosity
rro=rho1;
unorth=0; usouth=0; veast=0; vwest=0; % Velocity boundary conditions
time=0.0; % Initial time
rad=0.15;xc=0.5;yc=0.7; % Initial drop size and location

% Numerical variables
nx=32; ny=32; % Numbers of cells
dt=0.00125; nstep=100; % Time step size & number of time steps
maxit=200; maxError=0.001; beta=1.2; % Iteration parameters for pressure equation
...
%================== START TIME LOOP======================================
 for is=1:nstep,is
    ...
    u(1:nx+1,1)=2*usouth-u(1:nx+1,2);u(1:nx+1,ny+2)=2*unorth-u(1:nx+1,ny+1); % Tangential x-velocities at north and south bounds
    v(1,1:ny+1)=2*vwest-v(2,1:ny+1);v(nx+2,1:ny+1)=2*veast-v(nx+1,1:ny+1); % Tangential y-velocities at east and west bounds

    for i=2:nx
        for j=2:ny+1      % TEMPORARY u-velocity (accounting for advection, diffusion, and gravity terms)
    ...
    for i=2:nx+1
        for j=2:ny       % TEMPORARY v-velocity (accounting for advection, diffusion, and gravity terms)
...
%=======ADVECT DENSITY using centered difference plus diffusion ==========
    ...
    for i=2:nx+1
        for j=2:ny+1
          r(i,j)=ro(i,j)-(0.5*dt/dx)*(u(i,j)*(ro(i+1,j)...  % Compute advected density field based on new velocities and pressures
